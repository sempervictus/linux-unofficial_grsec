...
out_busy_undo:
	...
	BUG_ON(chk_cmd->scsi_atomic_blockers != 0);
...
static bool scst_check_blocked_dev(struct scst_cmd *cmd)
{
	...
	if (unlikely(cmd->internal || cmd->bypass_blocking)) {
		...
		BUG_ON((dev->on_dev_cmd_count == 0) && (cmd->cdb[0] != INQUIRY));
...
#ifdef CONFIG_SCST_EXTRACHECKS
	BUG_ON(test_bit(SCST_CMD_INC_EXPECTED_SN_PASSED, &cmd->cmd_flags));
	...
	bool other_ini, bool call_dev_task_mgmt_fn_received)
{
	...
	BUG_ON((cmd->cdb[0] == EXTENDED_COPY) && cmd->internal);
