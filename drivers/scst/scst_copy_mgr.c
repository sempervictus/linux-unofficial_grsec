...
static void scst_cm_write_cmd_finished(struct scst_cmd *wcmd)
{
	...
	if (likely(rc == SCST_CM_STATUS_CMD_SUCCEEDED))
		...
		if (rc != 0) {
			...
			BUG_ON(priv->cm_error == SCST_CM_ERROR_NONE);
		} else
...
out_check_retry:
	...
	BUG_ON(rc == SCST_CM_STATUS_CMD_SUCCEEDED);
...
out_err_done:
	BUG_ON((rc != -ENOENT) && !ec_cmd->completed);
